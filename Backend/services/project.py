...
from app import app
from services.db_services import execute,commitConnection
...
def createProject(id=None):
    try:
        ...
        validation_error = validateProjectData(name,start_date,department,manager)
        if validation_error:
            return validation_error
        project = Project(id, name,  start_date, department,manager)
        if name and start_date and department and manager and request.method == 'POST':
            sqlQuery = "INSERT INTO project(name,  start_date, department, manager) VALUES( %s, %s, %s,%s)"
            bindData = (project.name, project.start_date, project.department, project.manager)
...
def getProject():
    try:       
        ...
        print(f'Error: {e}')
        return jsonify({"error": "Could not retrieve projects from database"}), 500
    finally:
        cursor.close()
        conn.close()
...
def updateProject(id):
    try:
        ...
        project = Project(id, name,  start_date, department,manager )
        print(project.id)
        if id and name and start_date and department and manager  and request.method == 'PUT':
            ...
            if data == 0:
                ...
                sqlQuery = " UPDATE project SET name= %s, start_date= %s, department= %s,manager= %s WHERE id=%s "
                bindData = (project.name, project.start_date, project.department,project.manager, project.id)
                execute(sqlQuery, bindData)
                commitConnection()
...
@tocken_required
def deleteProject(id, name=None,  start_date=None, department=None,manager=None):
    try:
        project = Project(id, name,  start_date, department, manager)
...
#   def demo(id, name, start_date, department,manager,request):
#         if not name or not start_date or not department or not manager:
#         response = make_response(jsonify({'message': 'All fields are required'}))
#         response.status_code = 400
#         return response

#     project = Project(id, name, start_date, department,manager,request)
